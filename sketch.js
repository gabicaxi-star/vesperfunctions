...
function setupInputs() {
  ...
  inputs.occasionOther = createInput();
  ...
  // Crafts
  inputs.craftCandle = createCheckbox("Candle Making", false);
  inputs.craftJewelry = createCheckbox("Jewelry / Charm Making", false);
  inputs.craftSoap = createCheckbox("Soap Making", false);
  inputs.customCraft = createElement("textarea");
...
function drawStep0() {
  ...
  textSize(13);
  fill(80);
  text("This helps us understand the vibe and timing of your gathering.", x, y + 32);

  fill(COL_TEXT);
  textSize(14);
  text("Occasion *", x, y + 80);
  positionInput(inputs.occasion, x, y + 100, 260)

  if (form.occasion === "Other") {
    text("Please describe the occasion", x, y + 140);
    positionInput(inputs.occasionOther, x, y + 160, 320);
  }

  text("Preferred Date *", x, y + 210);
  positionInput(inputs.date, x, y + 230, 200);

  text("Preferred Time * (between 9:00 AM and 7:00 PM)", x + 260, y + 210);
  positionInput(inputs.time, x + 260, y + 230, 200);

  fill(90);
  textSize(12);
  text("*Your requested date will be approved by staff to prevent double-bookings.\nAvailability is not guaranteed until first deposit is paid.", x, y + 280);
...
function sendEmail() {
  ...
  emailjs.send("YOUR_SERVICE_ID_HERE", "template_trnx5um", {
